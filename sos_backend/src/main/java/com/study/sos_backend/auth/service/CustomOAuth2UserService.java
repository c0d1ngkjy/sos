package com.study.sos_backend.auth.service;

import com.study.sos_backend.auth.info.CustomOAuth2User;
import com.study.sos_backend.auth.info.OAuthAttributes;
...
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        ProviderType providerType = getSocialType(registrationId);
        ...
        User createdUser = getUser(extractAttributes, providerType); // getOwner() 메소드로 User 객체 생성 후 반환

        // DefaultOAuth2User 를 구현한 CustomOAuth2User 객체를 생성해서 반환
        return new CustomOAuth2User(
                Collections.singleton(new SimpleGrantedAuthority(createdUser.getRoleType().getCode())),
                attributes,
    ...
    private ProviderType getSocialType(String registrationId) {
    ...
    private User getUser(OAuthAttributes attributes, ProviderType providerType) {
        ...
        return userRepository.findBySocialIdAndProviderType(attributes.getOauth2UserInfo().getId(), providerType).orElseGet(() -> saveUser(attributes, providerType));
    }
